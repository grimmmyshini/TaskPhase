...
def main():
    ...
    for i in range(0, h): #<= for pixel rows of template image 
          ...
          row_cnt_strt = 0 if ( i < winsize/2 ) else ( i - winsize/2 )
          row_cnt_end = h if ( (h - i) < winsize/2 ) else ( i + winsize/2)    
           
                                
          for j in range(0, w): # <= for pixel columns o=f template image 
              
              edge = False
              clmn_temp_strt = 0 if ( j < winsize/2 ) else ( j - winsize/2 )
              clmn_temp_end = w if ( (w - j) < winsize/2 ) else ( j + winsize/2 )

#              print "heyyy" + str(clmn_temp_end - clmn_temp_strt) 
              
              temp_r = imgr_m[row_cnt_strt:row_cnt_end, clmn_temp_strt:clmn_temp_end] 

              search_max_l = 0 if ( j < dispmax ) else ( j - dispmax ) 
              search_max_r = w if ( (w-j) < dispmax ) else ( j + dispmax )
      
              for l in range( search_max_l, search_max_r): #<= for matching window

                  m += 1
#                  clmn_match_strt = 0 if ( l < winsize/2 ) else ( l - winsize/2 ) # hint! compare with j instead of winsize
#                  clmn_match_end = w if ( (w - l) < winsize/2 ) else ( l + winsize/2 )

                  clmn_match_strt = search_max_l if ((l -search_max_l) < (clmn_temp_end-clmn_temp_strt)/2) else (l - ((clmn_temp_end-clmn_temp_strt)/2))
                  clmn_match_end = search_max_r if ((search_max_r - l) < (clmn_temp_end-clmn_temp_strt)/2) else (l + ((clmn_temp_end-clmn_temp_strt)/2)) 
